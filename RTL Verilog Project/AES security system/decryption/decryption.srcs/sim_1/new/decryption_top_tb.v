...
module decryption_top_tb();
    ...
    initial begin
        ...
        rst = 1;
        key_in = 32'h0;
        cipher_text_in = 32'h0;

        // Apply reset
        #20 rst = 0;

        // Feed 128-bit key and ciphertext over 4 clock cycles (MSB to LSB)
        @(posedge clk); key_in = 32'h00000000; cipher_text_in = 32'h11223344;
        @(posedge clk); key_in = 32'h00000000; cipher_text_in = 32'h55667788;
        @(posedge clk); key_in = 32'h00000000; cipher_text_in = 32'h9900aabb;
        @(posedge clk); key_in = 32'h00000000; cipher_text_in = 32'hccddeeff;
       ...
       while (!done_flag) begin
        ...
        // Display results
        $display("\n==================== Final Result ====================");
        $display("Decrypted Plaintext : %h", plain_text);
